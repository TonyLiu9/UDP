...
while (1)
{
    if ((n = recvfrom(sockSrv, recvBuf, 500, 0, (SOCKADDR*)&addrClient, &len)) < 0) //recive the data from the server
    {
...
CK:while ((data_size = fread(dest, 1, MAX_LEN, tempfile)) > 0) //read 500 times and each time read a char to ausure the data size
{
   ...
   // std::cout << data_size;
    ...
    if (sendto(sockClient, itoa2(Checkflag), 500, 0, (SOCKADDR*)&addrSrv1, sizeof(SOCKADDR)) == -1)
    {
        std::cout << stderr << "sendto error" << std::endl;
        throw - 1;
    }
...
if (sendto(sockClient, itoa2(Checkflag), strlen(itoa2(Checkflag)), 0, (SOCKADDR*)&addrSrv1, sizeof(SOCKADDR)) == -1)
{
    std::cout << stderr << "sendto error" << std::endl;
    throw - 1;
}
P2:std::cout << "Transmit:" << filename << "     byte:" << data_size << "    flag:" << Checkflag - 2 << std::endl;
LISTEN:if ((n = recvfrom(sockSrv, recvBuf, 500, 0, (SOCKADDR*)&addrClient, &len)) < 0) //recive the data from the server
{
